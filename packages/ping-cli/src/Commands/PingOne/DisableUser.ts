/**
 * Disable user command for PingOne CLI
 *
 * @since 0.0.1
 */
...
import { updatePingOneUserStatus } from "../../HttpClient/PingOneClient.js"
...
/**
 * Command to disable a user in PingOne.
 *
 * @since 0.0.1
 */
export const disableUser = Command.make(
  "disable_user",
  {
    userId,
    environmentId,
  ...
  ({ userId, environmentId, pingoneToken }) =>
    Effect.gen(function*() {
      ...
      const response = yield* updatePingOneUserStatus({
        envId,
        token,
        userId,
        enabled: false
      ...
      yield* Console.log("User disabled successfully!")
