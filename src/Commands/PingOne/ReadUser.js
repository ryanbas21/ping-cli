...
exports.readUser = cli_1.Command.make("read_user", {
    userId,
    environmentId,
    pingoneToken
    ...
    // Read the user
    return yield* (0, PingOneClient_js_1.readPingOneUser)({
        envId,
        token,
        userId
    }).pipe(effect_1.Effect.flatMap((user) => Console.log(`User found!
ID: ${user.id}
...
Environment ID: ${user.environment.id}`)), effect_1.Effect.catchAll((error) => Console.error(`Failed to read user: ${error}`)));
}));
