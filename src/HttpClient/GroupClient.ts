...
export const createGroup = <S extends Schema.Schema.Type<typeof PingOneCreateGroupRequest>>(
  ...
  HttpClientRequest.post(`https://api.pingone.com/v1/environments/${envId}/groups`)
    .pipe(
...
export const readGroup = ({ envId, token, groupId, expand }: ReadGroupPayload) =>
  Effect.gen(function*() {
    const baseUrl = `https://api.pingone.com/v1/environments/${envId}/groups/${groupId}`
...
export const listGroups = ({ envId, token, limit, filter, expand }: ListGroupsPayload) =>
  Effect.gen(function*() {
    const baseUrl = `https://api.pingone.com/v1/environments/${envId}/groups`
...
export const updateGroup = <S extends Schema.Schema.Type<typeof PingOneUpdateGroupRequest>>(
  ...
  HttpClientRequest.patch(`https://api.pingone.com/v1/environments/${envId}/groups/${groupId}`)
    .pipe(
      HttpClientRequest.bearerToken(token),
      HttpClientRequest.accept("application/json"),
      HttpClientRequest.setHeader("Content-Type", "application/json"),
...
export const deleteGroup = ({ envId, token, groupId }: DeleteGroupPayload) =>
  Effect.gen(function*() {
    const req = HttpClientRequest.del(`https://api.pingone.com/v1/environments/${envId}/groups/${groupId}`).pipe(
      HttpClientRequest.bearerToken(token),
      HttpClientRequest.accept("application/json")
    )
...
export const addGroupMember = ({ envId, token, groupId, userId }: AddGroupMemberPayload) =>
  Effect.gen(function*() {
    const req = HttpClientRequest.post(
      `https://api.pingone.com/v1/environments/${envId}/groups/${groupId}/users/${userId}`
    ).pipe(
      HttpClientRequest.bearerToken(token),
      HttpClientRequest.accept("application/json")
    )
...
export const listGroupMembers = ({ envId, token, groupId, limit }: ListGroupMembersPayload) =>
  Effect.gen(function*() {
    const baseUrl = `https://api.pingone.com/v1/environments/${envId}/groups/${groupId}/users`
